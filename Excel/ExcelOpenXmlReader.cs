	...
	public class ExcelOpenXmlReader : IExcelDataReader
	{
		...
		private bool ReadSheetRow(XlsxWorksheet sheet)
		{
                ...
                _xmlReader.ReadToFollowing(XlsxWorksheet.N_row))
			{
                ...
                //System.Diagnostics.Debug.WriteLine(string.Format("{0} - {1}",sheet.Name, rowIndex.ToString()));
				...
				int row = 0;
 				...
 				while (_xmlReader.Read())
 				{
					...
					if (_xmlReader.NodeType == XmlNodeType.Text && hasValue)
					{
                        ...
                        {
                            System.Diagnostics.Debug.WriteLine(ex.Message + (string.Format("\n{0} Row {1}, Column {2}, Value {3}", sheet.Name, row.ToString(), col.ToString(), o.ToString())));
                            throw new Exception(ex.Message + (string.Format("\n{0} - {1} {2} = {3}", sheet.Name, row.ToString(), col.ToString(), o.ToString())));
                        }
					}
				}
